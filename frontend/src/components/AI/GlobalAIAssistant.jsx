...
export default function GlobalAIAssistant() {
  ...
  const PaletteModal = isOpen ? createPortal(
    <div className="fixed inset-0 z-[200] flex items-start justify-center pt-[15vh]">
      <motion.div 
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="absolute inset-0 bg-space-950/60 backdrop-blur-sm"
      ...
      <motion.div
        ...
        className={`relative w-full max-w-xl mx-4 rounded-xl shadow-2xl border overflow-hidden ${
      ...
      >
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2">
          ...
          <div className="px-3 pb-2 pt-2">
            <p className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2 px-1">Actions rapides</p>
            {filteredActions.length === 0 ? (
              ...
              filteredActions.map(action => (
                ...
                >
                  <div className={`p-2 rounded-md ${isDark ? 'bg-space-800' : 'bg-gray-100'}`}>
                    <action.icon className="w-4 h-4 text-gray-400" />
                  </div>
                  <div>
                    <h4 className={`text-sm font-semibold ${isDark ? 'text-gray-200' : 'text-gray-800'}`}>{action.label}</h4>
                    <p className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-500'}`}>{action.desc}</p>
                  </div>
                </button>
              ))
            )}
