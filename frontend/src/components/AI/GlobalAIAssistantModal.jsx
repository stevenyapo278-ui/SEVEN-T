...
import { 
  ...
  LayoutDashboard, Bell, Radio, ArrowRight
} from 'lucide-react'
...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'SET_MAGIC_ACTION': 
      return { ...state, magicAction: action.value }
    case 'SET_RECORDS': 
...
export default function GlobalAIAssistantModal({ isOpen, onClose }) {
  ...
  const { query, activeIndex, isExecuting, magicAction, records } = state
  ...
  const fetchRecords = async () => {
    if (isLoadingRecords.current) return
  ...
  useEffect(() => {
    ...
    if (q.length < 3) {
      dispatch({ type: 'SET_MAGIC_ACTION', value: null });
      return;
    }
      ...
      <span>
        {parts.map((part, i) => 
          part.toLowerCase() === query.toLowerCase() 
            ? <b key={`highlight-${i}-${part}`} className="text-gold-400 font-extrabold">{part}</b> 
            : part
        )}
      </span>
    );
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault();
      dispatch({ type: 'SET_ACTIVE_INDEX', value: (activeIndex + 1) % totalItems });
    } else if (e.key === 'ArrowUp') {
      e.preventDefault();
      dispatch({ type: 'SET_ACTIVE_INDEX', value: (activeIndex - 1 + totalItems) % totalItems });
    } else if (e.key === 'Enter') {
      e.preventDefault();
  ...
  return createPortal(
    <div className="fixed inset-0 z-[999] flex items-start justify-center pt-[10vh] sm:pt-[15vh]">
      ...
      >
        <div className="flex items-center border-b p-4 gap-3 relative">
          ...
          <input
            ...
            className={`flex-1 bg-transparent border-none outline-none text-lg ${
              isDark ? 'text-white placeholder-gray-500' : 'text-zinc-900 placeholder-gray-400'
            }`}
            placeholder="Que voulez-vous faire ?"
          />
          <button onClick={onClose} className={`p-1 rounded-md ${isDark ? 'hover:bg-space-800' : 'hover:bg-zinc-100'}`}>
             ...
             <div className="px-1 mb-4">
                ...
                >
                    ...
                    <div className="flex-1">
                        <p className={`text-sm font-bold ${isDark ? 'text-white' : 'text-zinc-900'}`}>{magicAction.label}</p>
                        <p className={`text-xs ${isDark ? 'text-emerald-500/70' : 'text-emerald-600/70'}`}>{magicAction.desc}</p>
                    </div>
                    {isExecuting && <div className="animate-spin rounded-full size-4 border-2 border-emerald-500 border-t-transparent" />}
                </button>
             </div>
          )}
          ...
          {query.trim() && (
             <div className="px-1 mb-4">
                ...
                >
                    ...
                    <div>
                        <p className={`text-sm font-semibold ${isDark ? 'text-zinc-100' : 'text-zinc-900'}`}>"{query}"</p>
                        <p className={`text-xs ${isDark ? 'text-zinc-500' : 'text-zinc-500'}`}>Demander à l'assistant de configurer ça pour vous</p>
                    </div>
                 </button>
             </div>
          )}
          ...
          <div className="px-1 pb-4">
            {filteredActions.length === 0 && !magicAction && totalFilteredRecords === 0 ? (
              <div className="py-12 text-center">
                 <div className={`size-16 rounded-2xl mx-auto mb-4 flex items-center justify-center ${isDark ? 'bg-space-800' : 'bg-zinc-100'}`}>
                    <Search className="size-8 text-zinc-500" />
                 </div>
                 <p className="text-sm text-zinc-500">Aucune action ou donnée trouvée pour "{query}"</p>
              </div>
            ) : (
              <>
                {filteredActions.reduce((acc, action, idx) => {
                  ...
                  if (showCategory) {
                    acc.push(
                      <p key={`cat-${action.category}`} className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-2 mt-4 px-3 first:mt-1">
                        {action.category}
                      </p>
                    );
                  }
                  ...
                  acc.push(
                    <button
                      ...
                      className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all ${
                        itemIdx === activeIndex
                          ? (isDark ? 'bg-space-800 ring-1 ring-white/10 shadow-lg' : 'bg-zinc-100 ring-1 ring-black/5 shadow-md')
                    ...
                    >
                      <div className={`p-2 rounded-lg flex-shrink-0 transition-colors ${
                        itemIdx === activeIndex 
                          ...
                          : (isDark ? 'bg-space-800' : 'bg-zinc-100')
                      }`}>
                        <action.icon className={`size-4 transition-colors ${
                          itemIdx === activeIndex ? 'text-gold-400' : 'text-zinc-400'
                        }`} />
                      </div>
                      <div className="min-w-0 flex-1">
                        <h4 className={`text-sm font-semibold truncate ${
                          itemIdx === activeIndex ? (isDark ? 'text-white' : 'text-zinc-900') : (isDark ? 'text-zinc-100' : 'text-zinc-800')
                        }`}>{highlightMatch(action.label, query)}</h4>
                        <p className={`text-[10px] truncate ${
                          itemIdx === activeIndex ? 'text-zinc-400' : 'text-zinc-500'
                ...
                {filteredRecords.products.length > 0 && (
                  <div className="mt-4">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-2 px-3">Produits du catalogue</p>
                    {filteredRecords.products.map((p, i) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-all ${
                            itemIdx === activeIndex ? (isDark ? 'bg-space-800 shadow-lg' : 'bg-zinc-100 shadow-md') : 'hover:bg-space-800/20'
                          }`}
                        >
                          ...
                          <div>
                            <p className="text-sm font-medium text-zinc-100">{highlightMatch(p.name, query)}</p>
                            <p className="text-[10px] text-zinc-500">{p.price} FCFA · Stock: {p.stock}</p>
                          </div>
                        </button>
                      );
                    })}
                ...
                {filteredRecords.leads.length > 0 && (
                  <div className="mt-4">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-2 px-3">Contacts CRM</p>
                    {filteredRecords.leads.map((l, i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-sm font-medium text-zinc-100">{highlightMatch(l.name, query)}</p>
                            <p className="text-[10px] text-zinc-500">{l.phone || 'Pas de numéro'}</p>
                          </div>
                        </button>
                      );
                    })}
                ...
                {filteredRecords.agents.length > 0 && (
                  <div className="mt-4">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-500 mb-2 px-3">Assistantes IA</p>
                    {filteredRecords.agents.map((a, i) => {
                      ...
                      return (
                        ...
                        >
                          <div className="size-8 rounded-lg bg-gold-400/10 flex items-center justify-center">
                            <Bot className="size-4 text-gold-400" />
                          </div>
                          <div>
                            <p className="text-sm font-medium text-zinc-100">{highlightMatch(a.name, query)}</p>
                            <p className="text-[10px] text-zinc-500">{a.role || 'Agent IA'}</p>
                          </div>
                        </button>
                      );
                    })}
        ...
        <div className={`px-4 py-2 border-t ${isDark ? 'border-white/5 text-zinc-600' : 'border-gray-100 text-zinc-400'} flex items-center justify-between text-[10px] font-bold uppercase tracking-widest`}>
           <div className="flex items-center gap-4">
             <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-zinc-400">↑↓</kbd> Naviguer</span>
             <span className="flex items-center gap-1"><kbd className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-zinc-400">↵</kbd> Valider</span>
           </div>
           <div className="flex items-center gap-1">
             <kbd className="px-1.5 py-0.5 rounded bg-white/5 border border-white/10 text-zinc-400">esc</kbd> Fermer
           </div>
        </div>
      </m.div>
    </div>,
